...
export class BSubmap {
    ...
    addInput(name: string, typeName: string, array: boolean = false) {
        this.inputs.push(new BSubmapField(this.vmapping, name, typeName, this.name, false, array));
    }

    setOutput(name: string, typeName: string, array: boolean = false) {
        this.output = new BSubmapField(this.vmapping, name, typeName, this.name, true, array);
    }
    ...
    draw(assignedX: number, assignedY: number): JSX.Element {
        ...
        <text x={startX + fwidth - 300} y={startY + 70 + this.inputs.length * 30} className="fieldName">{this.output.name} : {this.output.typeName}{this.output.array? " []" : ""}</text>
        ...
        return <>
        <rect x={startX} y={startY} rx="10" ry="10" width={fwidth} height={fhieght} fill="lightblue" stroke="black" onMouseDown={(e) => {funcMoustDown(e, this)}} />
...
const funcMoustDown = (e: MouseEvent<SVGRectElement>, bfunc: BSubmap) => {
    e.preventDefault();
...
export class BSubmapField extends BField {
